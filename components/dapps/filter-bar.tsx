    ...
    <div className="flex flex-wrap items-center gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent>
            <DropdownMenuItem onClick={() => onCategoriesChange([])}>
              All Categories
              {selectedCategories.length === 0 && <Check className="ml-2 h-4 w-4" />}
            </DropdownMenuItem>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            {sortOptions.map((option) => (
              <DropdownMenuItem key={option.value} onClick={() => onSortByChange(option.value as any)}>
                {option.label}
      ...
      {selectedCategories.length > 0 && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="ghost" size="sm" className="h-7" onClick={() => onCategoriesChange([])}>
            Clear all
          </Button>
        </div>
      )}
