...
import { AlertTriangle, ArrowRightLeft, Check, ExternalLink, Loader2, SendHorizontal } from "lucide-react"
...
export function TransactionConfirmModal({ isOpen, onClose, onConfirm, transaction }: TransactionConfirmModalProps) {
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <div className="rounded-full bg-primary/10 p-1">{renderTransactionIcon()}</div>
            {renderTransactionTitle()}
          </DialogTitle>
          <DialogDescription>{renderTransactionDescription()}</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    <span className="text-lg font-medium">{transaction.fromToken?.amount}</span>
                    <Badge variant="outline">{transaction.fromToken?.symbol}</Badge>
                  </div>
                  {transaction.fromToken?.value && (
                    <p className="text-sm text-muted-foreground">(≈${transaction.fromToken.value})</p>
                  )}
                </div>
                ...
                <div className="text-right">
                  ...
                  <div className="flex items-center gap-2 justify-end">
                    <span className="text-lg font-medium">{transaction.toToken?.amount}</span>
                    <Badge variant="outline">{transaction.toToken?.symbol}</Badge>
                  </div>
                  {transaction.toToken?.value && (
                    <p className="text-sm text-muted-foreground">(≈${transaction.toToken.value})</p>
                  )}
                </div>
              </div>
                  ...
                  {transaction.routeInfo && (
                    <div className="mt-2">
                      ...
                      <div className="flex items-center gap-1 mt-1 text-sm">
                        {transaction.routeInfo.route.map((step, index) => (
                          <React.Fragment key={index}>
                            <Badge variant="outline" className="text-xs">
                              {step}
                            </Badge>
                            {index < transaction.routeInfo!.route.length - 1 && (
                              <ArrowRightLeft className="h-3 w-3 text-muted-foreground mx-1" />
                            )}
                          </React.Fragment>
                        ))}
                      </div>
                      <p className="text-xs text-muted-foreground mt-1">Fee: {transaction.routeInfo.fee}</p>
                    </div>
                  )}
                </div>
              )}
          ...
          <div>
            <div className="flex justify-between py-1">
              ...
              <p className="text-sm font-medium">{transaction.network}</p>
            </div>
            <div className="flex justify-between py-1">
              <p className="text-sm text-muted-foreground">Estimated Gas</p>
              <p className="text-sm font-medium">{transaction.estimatedGas}</p>
            </div>
          </div>

          <Separator />

          <Alert variant="outline" className="border-yellow-500/20 bg-yellow-500/10">
            <AlertTriangle className="h-4 w-4 text-yellow-500" />
            <AlertDescription className="text-sm text-yellow-500">
              Always verify transaction details before confirming. This cannot be undone.
            </AlertDescription>
          </Alert>
        </div>
