...
import { TransactionConfirmModal, type TransactionDetails } from "./transaction-confirm-modal"
...
export function AssistantLayout() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "welcome",
      content: "Hello! I'm your Soneium AI Assistant. How can I help you today?",
      sender: "ai",
      timestamp: new Date(),
    },
  ])
  ...
  useEffect(() => {
    ...
    document.addEventListener("dismissAssistantTooltip", handleDismissTooltip)

    return () => {
      document.removeEventListener("dismissAssistantTooltip", handleDismissTooltip)
    }
  }, [])
  ...
  const handleSendMessage = (content: string) => {
    ...
    if (
      (lowerCaseMessage.includes("send") || lowerCaseMessage.includes("transfer")) &&
      ...
      const amountMatch = isEth ? content.match(/(\d+\.?\d*)\s*eth/i) : content.match(/(\d+\.?\d*)\s*usdc/i)
      ...
      setTransactionDetails({
        ...
        fromToken: {
          ...
          amount: amount,
          value: isEth ? (Number.parseFloat(amount) * 3000).toString() : amount,
        },
        recipient: recipient,
      ...
      setTimeout(() => {
        const aiMessage: Message = {
          id: `ai-${Date.now()}`,
          content: `I'll help you transfer ${amount} ${isEth ? "ETH" : "USDC"} to ${recipient}. Please review and confirm this transaction in the confirmation window.`,
          sender: "ai",
          timestamp: new Date(),
        }
  ...
  const getAIResponse = (userMessage: string, model: AIModel): string => {
    ...
    if (lowerCaseMessage.includes("hello") || lowerCaseMessage.includes("hi")) {
      return modelInfo + "Hello! How can I assist you with your Web3 journey today?"
    } else if (lowerCaseMessage.includes("gas") || lowerCaseMessage.includes("fee")) {
      return (
        modelInfo +
        "Current gas fees on Ethereum are around 25 Gwei. Would you like me to help optimize your transaction?"
      )
    } else if (lowerCaseMessage.includes("swap") || lowerCaseMessage.includes("exchange")) {
      return (
        modelInfo +
        "I can help you swap tokens. Please tell me which tokens you'd like to exchange and the amount. For example, 'Swap 0.1 ETH to USDC'."
      )
    } else if (lowerCaseMessage.includes("transfer") || lowerCaseMessage.includes("send")) {
      return (
        modelInfo +
        "I can help you transfer tokens. Please specify the token, amount, and recipient address. For example, 'Send 0.1 ETH to 0x1234...5678'."
      )
    } else if (lowerCaseMessage.includes("token") || lowerCaseMessage.includes("price")) {
      return modelInfo + "I can fetch token prices for you. Which specific token are you interested in?"
    } else if (lowerCaseMessage.includes("nft")) {
      return (
        modelInfo +
        "I can help you analyze NFT collections, check floor prices, or verify authenticity. What specific NFT information do you need?"
      )
    } else if (lowerCaseMessage.includes("wallet")) {
      return (
        modelInfo +
        "I can help you manage your wallet, check balances, or analyze transaction history. What would you like to know about your wallet?"
      )
    } else if (lowerCaseMessage.includes("model") || lowerCaseMessage.includes("which model")) {
      return `I'm currently using ${model.name} by ${model.provider}. This model ${model.description.toLowerCase()} and has a context length of ${model.contextLength.toLocaleString()} tokens. You can change the model using the selector at the top of the chat.`
    } else {
      return (
        modelInfo +
        "I understand you're asking about Web3. Could you provide more details about what you'd like to know or do?"
      )
    }
  }
